...
The deployment.yaml will be feching this image and will deploy it to EKS using rolling updates in the respective namespace.

Pre-requisite:
1. Jenkins server should be up and running.
2. Cluster is up and running. cluster[0] will be taken dynamocally via jenkins pipeline using cli command.
...
1. Open Global credentials in Jenkins and add below configs:
	i. Select Username and Password - credentials shared separately over chat - use id as 'sd-dockerhub-creds'
	ii. Two AWS credentials - use id as 'sd-aws-creds-dev' and 'sd-aws-creds-prod'
2. On jenkins homepage, click on new item and create a pipeline for prod and dev using main and dev in the branch section.
3. Once pipeline is created, go to configure and set 'Pipeline script from SCM' and use below git url and use the branch respectively. ('*/main' for prod pipeline and '*/dev' for dev pipeline).
...
5. Run the pipeline. It should push the image to docker and deploy to eks. Rolling deployment is used while deploying a new image to eks. 
6. Deployment/service will be created in respective namespaces depending on the branch name. 
